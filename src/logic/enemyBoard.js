// Takes in an array of ship types to place (if none is provided, uses a default set):
export const enemyBoard = typesArray => {
  const ships = [];
  ...
  // If an array of ship types is not passed in, a default set of ship types is provided:
  if (typesArray) {
    typesArray.forEach(type => {
      placeShip(type);
    });
  } else {
    const defaultTypes = [5, 4, 6, 3, 2];
  ...
  // If it's easier to work with the same ship placement during development, comment out accordingly:
  return ships;
  // return [ 32526, 32536, 32546, 32556, 32566, 32483, 32484, 32485, 32486, 32334, 32344, 32354, 32612, 32622, 32623, 32209, 32200 ];
};
